...
class CursorBasedQueryTest {
  ...
  @Test
  void testSetName() {
    CursorBasedQuery query = CursorBasedQuery.builder().first(3).build();
    assertEquals(3, query.getFirst(), "unexpected first");
  }

  @Test
  void testSetAfter() {
    CursorBasedQuery query = CursorBasedQuery.builder().after("test").build();
    assertEquals("test", query.getAfter(), "unexpected after");
  }

  @Test
  void testToBuilder() {
    CursorBasedQuery query = CursorBasedQuery.builder().first(3).after("test").build();

    CursorBasedQuery copy = query.toBuilder().first(4).build();

    CursorBasedQuery expected = CursorBasedQuery.builder().first(4).after("test").build();
